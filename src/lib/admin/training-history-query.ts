...
/** Oefeningen die deze klant ooit gelogd heeft, meest recent eerst. */
export async function listLoggedExercisesForMember(
  profileId: string,
  ...
  const { data, error } = await admin
    ...
    .select(
      `
        ...
        session:workout_sessions!inner(started_at, completed_at, profile_id)
      `,
    )
  ...
  if (error) {
    console.error("[listLoggedExercisesForMember] query failed", error);
    return [];
  }
...
/** Volledige log-historie van deze klant voor één oefening, chronologisch. */
export async function loadExerciseHistory(
  ...
  const { data, error } = await admin
    ...
    .select(
      `
        session_id, set_number, weight_kg, reps, exercise_id,
        session:workout_sessions!inner(id, started_at, completed_at, profile_id)
      `,
    )
  ...
  if (error) {
    console.error("[loadExerciseHistory] query failed", error);
    return [];
  }
