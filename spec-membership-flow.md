...
- **Klantbeheer-workstream**: fase 1 (pauzeren/hervatten, PR #88 `688b5ba`) en fase 2A (admin-stop, PR #89 `893106f`) zijn GEMERGED; discovery in `discovery-klantbeheer-lifecycle.md` (PR #87, gemerged, `1db25c4`). Fase 2B (change-requests + e-mailcorrectie) zit in de PR van branch `feat/lifecycle-change-requests-email` (zie de ledger-regel hierboven); daarmee is de gedeelde primitieven-laag compleet en resteren alleen de voorkanten (admin-UI en lid-UI, na de mockup-ronde, Sonnet). Adres bijwerken bestaat al (`updateProfile`, puur profielveld, geen Mollie-operatie).
