...
- **WS-5 (admin betaalverzoek-systeem)**: mockup goedgekeurd (Nieuw-betaalverzoek-wizard: klant zoeken/aanmaken, catalogus-product, betaallink per e-mail + kopieerbare link). PR A en PR B gemerged; PR C gebouwd en staat open, enige resterende actieve WS-5-PR:
  - **PR C (Sonnet) wizard-UI**: GEBOUWD, PR #82 (feat/ws5-betaalverzoek-wizard) staat OPEN. Route /app/admin/betaalverzoeken (erft de requireAdmin-guard), vier stappen met stepper: Klant (searchCustomers-zoek of nieuwe-klant-aanmaak, UI-vangrail spiegelt PR B's dubbele-e-mailcheck en "bestaat al"-afhandeling), Product (abonnement = de 7 configurator-opties met vt-/ext-toggles, of los product uit dezelfde vijf slugs als WS-6, geen drop-in; custom bedrag zichtbaar-uitgeschakeld), Voorwaarden (waiver + 12/24m bij abonnement, niets bij een product), Versturen (TTL 1/3/7/14 dagen, createPaymentRequest, kopieerbare link). Prijslogica uitsluitend hergebruikt uit abonnement/lib.ts (computeBreakdown, planSlug); geen nieuwe prijsformules, geen wijziging aan createPaymentRequest/findOrCreateCustomer/admin_create_order/_compute_order_price. De publieke ConfigureStage-UI zelf is niet hergebruikt (full-bleed breakout botst met de vaste admin-sidebar); nieuwe compacte rijen in de bestaande admin-vocabulaire. Chrome-fix hoort bij deze PR: /betaal/[token] kreeg via SiteShell.tsx de volle marketing-navbar/footer/EM-banner mee, nu uitgesloten (zelfde patroon als /app, /login, /checkin), betaal-logica zelf ongewijzigd, live geverifieerd met curl (geen nav/footer meer op /betaal, wel op de homepage). "Betaalverzoeken" toegevoegd aan AdminSidebar. tsc en eslint schoon; productie-build compileert de nieuwe route schoon (enige build-fout is de bekende, pre-existing /prijzen-lokale-env-beperking). De wizard zelf kon niet interactief in de browser doorlopen worden: alle /app/admin/*-routes geven lokaal 500 door de bekende ontbrekende Supabase-env (leden/trainers 500en identiek), geen regressie van deze PR.
