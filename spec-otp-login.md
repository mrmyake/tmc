...
- `src/components/checkout/IdentifyStage.tsx` (tot de /kopen-PR: `src/app/abonnement/IdentifyStage.tsx`) houdt de default (true). Dit is de enige component waar een nieuw account mag ontstaan, op precies twee routes: `/abonnement` en `/kopen` (publieke productcheckout, zie de ledger in `spec-membership-flow.md`). Op beide heeft de bezoeker net iets gekozen. Bewust afwijkend van `/login`; niet "consistent maken".
