...
**PR #207 (branch `feat/account-deletion-core`), 2026-09-21.** Workstream D.2, PR 2: de orchestratie, geen UI, geen schema. Module `src/lib/account-deletion/` (config, types, pure kern op geinjecteerde deps, wiring). Verzoek: voorwaarde is een opgezegd of afgelopen lidmaatschap (anders `membership_active` met verwijzing naar opzeggen; een verwijderverzoek start geen opzegging en raakt de betaalverplichting niet), dan snapshot van de externe ids in `tmc.account_deletions` en het plannen van sluiting en purge. Sinds PR #208: de sluiting (ban op de auth-user, Akiles-sync en device-tokens, push-tokens, toekomstige boekingen, wachtlijst, PT- en gastboekingen, tegoed vervallen, marketing) is direct als er geen lopend abonnement is, en wacht anders op de dag na de laatste betaalde dag van het opgezegde abonnement (cron); daarna start de bedenktijd. Tegoed is geen voorwaarde. Alleen de admin-hardstop zegt op en verwijdert in een handeling, via `cancelMembershipCore`. Purge door cron `process-account-deletions` (05:05, tijdsbudget per rij) in vaste volgorde: freeze, Mollie-subscriptions, Akiles (naam op pseudoniem, magic link en member weg), MailerLite, push, profiel (`anonymise_profile` bij orders of facturen, anders hard delete), afsluitmail vanaf het gesnapshotte adres; Mollie-customer dertien maanden na afronding. Alleen geld en deur blokkeren het profiel; MailerLite blokkeert nooit. `desired-state.ts` sluit de deur bij een open verwijderverzoek voor elke rol. Clients: `deleteMagicLink`/`deleteMember` (Akiles), `deleteMollieCustomer`, `findSubscriberIdByEmail`/`forgetSubscriber`. `deleteMember` en `requestAccountDeletion` gaan over op dezelfde kern; meldingen alleen met ids. Termijnen in `config.ts`; open voor Marlon: opzegtermijn, bedenktijd, MailerLite-modus, en opzeggen-eerst versus een gekoppelde knop. Bewust niet aangeraakt: de RPC uit #206, schema, UI (dialoog en admin-dialoog, PR 3 en 4), PT-sessies zelf, gast-mail, herstel na een geannuleerd verzoek.

**PR #208 (branch `feat/account-deletion-member-ui`), 2026-09-21.** Workstream D.2, PR 3: de ledenkant, kern en schema onaangeraakt. Nieuwe pagina `/app/profiel/verwijderen` (app en web): preflight met wat er direct verandert, wat bewaard blijft en waarom (opgezegd abonnement loopt door, facturen en betalingen zeven jaar) en de verwachte purge-datum via `computePurgeAfter`; bij een lopend lidmaatschap een voorwaarde-scherm met einddatum, knop naar opzeggen en de datum waarna de gegevens verdwijnen als het lid vandaag opzegt; bevestiging met een 6-cijferige code naar het adres van de sessie (server-side geverifieerd met `Sb-Forwarded-For`, gebonden aan de ingelogde gebruiker, `auth.otp_failed` bij falen); statusscherm met wat al is ingetrokken. Sectie op `/app/profiel` verwijst naar de pagina en toont een open verzoek; de dialoog en de tekst uit #205 zijn weg. Tweede commit, zelfde PR: de sluiting is verplaatst naar de dag na de laatste betaalde dag van het opgezegde abonnement (`runScheduledFreezeCore` via de cron; direct als er geen lopend abonnement is), zodat een lid dat heeft opgezegd maar nog betaalt tot die dag blijft inloggen, boeken en de deur openen; de toegangssync sluit pas bij `deletion_frozen`. Daardoor kan de status op de profielpagina blijven staan en trekt het lid het verzoek zelf in zolang de sluiting wacht (`cancelAccountDeletion` op `cancelDeletionRequestCore`). Tegoed (rittenkaart, PT-pakket) is uit de voorwaarde gehaald: niet opzegbaar, vervalt zonder restitutie bij de sluiting (`expireCreditRows`, status expired), het lid bevestigt dat in de preflight. Tests: 31 in `scripts/account-deletion/`. Bewust niet aangeraakt: de RPC uit #206, `auth.ts`, de admin-dialoog, de opzegflow.
...
**Vastgelegd bij PR #207 (2026-09-21): een verwijderverzoek beeindigt geen overeenkomst en vervalt geen betaalverplichting.** Account verwijderen en lidmaatschap opzeggen zijn twee beslissingen. Een lid kan het verwijderverzoek alleen indienen als het lidmaatschap is opgezegd (`cancellation_requested`) of al afgelopen is; loopt het nog, dan wijst de kern het verzoek af met `membership_active` en verwijst de app naar het opzegscherm. Dat is geen weigering van de verwijdering (opzeggen kan altijd, verwijderen daarna ook), dus het blijft binnen Apple 5.1.1(v). De purge wacht op de laatste van de ingangsdatum van de opzegging, de datum van de laatste factuur en de bedenktijd; gegevens die nodig zijn om een lopende overeenkomst uit te voeren blijven bewaard, naast de wettelijke bewaartermijn. Alleen de admin-hardstop (`deleteMember`, een handeling van Marlon met eigen afweging) zegt op en verwijdert in een handeling, via dezelfde kern. Wat het lid nog heeft betaald blijft bruikbaar: de sluiting (toegang, toestellen, boekingen, tegoed) gebeurt op de dag na de laatste betaalde dag van het opgezegde abonnement, of direct als er geen lopend abonnement is; daarna start de bedenktijd. Tegoed (rittenkaart, PT-pakket) is geen voorwaarde om te mogen verwijderen: het is niet opzegbaar en er wordt geen restitutie verleend; het vervalt bij de sluiting en het lid bevestigt dat vooraf.
...
**PR #209 (branch `feat/account-deletion-admin`), 2026-09-22.** Workstream D.2, PR 4, de laatste: admin en teksten. Nieuwe pagina `/app/admin/verwijderverzoeken` (sidebar-item onder Pauzes) met een lijst uit `tmc.account_deletions` (lid, status, moment van sluiting, purge-datum, per stap de uitkomst en de laatste fout), leesdeel in `src/lib/admin/account-deletion-query.ts`. Twee acties in `src/lib/admin/account-deletion-actions.ts`: verzoek intrekken (wrapper om de bestaande `cancelAccountDeletionRequest`) en een nieuwe `hardstopAccountDeletion` (wiring-laag, niet de kern) die elke nog levende abonnement-membership per direct cancelt en dan `runPurgeCore` aanroept, zodat een wachtende of vastgelopen sluiting meteen doorgezet wordt. `ActionMenu.tsx`: de verouderde tekst over handmatig de Mollie-subscription stoppen vervangen door wat er nu echt gebeurt bij `deleteMember`. Privacyverklaring sectie 06: bewaartermijnen per categorie (facturen en betaalgegevens 7 jaar, lidmaatschapsgegevens 7 jaar, profielgegevens en boekingen bij verwijdering geanonimiseerd, marketingtoestemming direct) plus dat een verwijderverzoek geen overeenkomst beeindigt en geen betaalverplichting laat vervallen. Algemene voorwaarden: nieuw artikel 15 "Account verwijderen" (klachten/wijziging/toepasselijk recht naar 16-18), en de tegoed-clausule in artikel 7 uitgebreid van alleen rittenkaarten naar ook PT-pakketten en andere losse aankopen. Stale verwijzingen bijgewerkt: dit document (D.2, de "Te ontwerpen"-paragraaf en de placeholder-copy gemarkeerd als gebouwd), `spec-ledenomgeving.md` 1.4 (beschreef zelf weer een stale tussenstand), `claude-code-prompts.md` (het nooit-gebouwde `profiles.deleted_at`-plan geannoteerd als achterhaald). Bijvangst: de tweede commit van #208 had een mislukte push en zat niet in de merge van die PR; deze branch begint met die twee commits alsnog, teruggehaald via cherry-pick. Bewust niet aangeraakt: de kern (`core.ts`), de RPC uit #206, `legal-source/` in de andere worktree, geen migraties, geen `db push`.
