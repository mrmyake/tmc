...
- **Klantbeheer-workstream**: fase 1 (pauzeren/hervatten, PR #88 `688b5ba`) en fase 2A (admin-stop, PR #89 `893106f`) zijn GEMERGED; discovery in `discovery-klantbeheer-lifecycle.md` (PR #87, gemerged, `1db25c4`). Resteert fase 2B (go gegeven 2026-07-12, in uitvoering): `membership_change_requests` (alleen-upgrade op de volgende factuurdatum, geen proratie) en de directe e-mailcorrectie op een bestaand account (auth.users.email + identities + profiles.email synchroon, zoek-eerst, weigeren bij bestaand ander account, volledige PR B-strengheid; wijzigt een login-adres, dus Fable). Adres bijwerken bestaat al (`updateProfile`, puur profielveld, geen Mollie-operatie).
