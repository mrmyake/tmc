...
**Kritieke pad (PR #TBD, 3a-bis).** De webhook antwoordt zodra `activate_order` gecommit is en het
...
- **PR #TBD, 2026-09-19** (branch `fix/webhook-critical-path`, geen migratie; `vercel.json`, `src/lib/outbound-timeouts.ts` nieuw, `src/lib/supabase/admin.ts` en `server.ts`, `src/lib/mollie.ts`, `src/lib/email.ts`, `src/lib/akiles.ts`, `src/lib/orders/activation-chain.ts`, `src/app/api/mollie/webhook/route.ts`, `scripts/outbound/timeouts.test.mts`, dit document 6.5, 12 en 15; cross-cutting regels in `spec-membership-flow.md`). 3a-bis van de webhook-betrouwbaarheid, drie commits: (1) functies vastgezet op `fra1` na vaststelling met `vercel inspect` dat ze in `iad1` draaiden, gemeten hop 0,2 tot 0,3 s per databasecall; (2) expliciete timeouts op Supabase, Mollie, MailerSend en Akiles, elk als gevangen fout van die dienst; (3) de vijf zijeffecten en de mail-retry achter de respons met `after()`, `maxDuration = 60`, subscription-pad synchroon, `runActivationChain` met `options.defer` zodat een cron hem inline blijft draaien. Besluit 36: de bevestigingsmail heeft daarmee geen webhook-vangnet meer, PR 3b pakt dat op. **Bewust niet aangeraakt:** de classificatie en responscodes uit #193, `activate_order` en alle andere RPC's, de reconciliatie-cron (PR 3b).
