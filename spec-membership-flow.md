...
- **WS-5 (admin betaalverzoek-systeem)**: mockup goedgekeurd (Nieuw-betaalverzoek-wizard: klant zoeken/aanmaken, catalogus-product, betaallink per e-mail + kopieerbare link). Na PR A en PR B resteert nog een PR:
  - **PR C (Sonnet) wizard-UI**: nog niet gestart, enige resterende actieve WS-5-PR. Doelbeeld = de goedgekeurde mockup. Consumeert PR A (createPaymentRequest) en PR B (findOrCreateCustomer, searchCustomers). Meenemen: besluit of /betaal het volledige site-chrome (navbar/footer met Early Member-banner) houdt of een kalere pagina wordt.
