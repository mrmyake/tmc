...
- **WS-5 PR C (admin Nieuw-betaalverzoek-wizard)**: GEMERGED (PR #82, merge `9a400c2`, commit `e2002ff`, branch feat/ws5-betaalverzoek-wizard) en END-TO-END GEVERIFIEERD op de preview: een productaankoop (pt_single) en een abonnementaankoop (all_inclusive_3x) zijn allebei via de wizard verzonden, betaald en geactiveerd. Route /app/admin/betaalverzoeken (erft de requireAdmin-guard), vier stappen met stepper: Klant (searchCustomers-zoek of nieuwe-klant-aanmaak, UI-vangrail spiegelt PR B's dubbele-e-mailcheck en "bestaat al"-afhandeling), Product (abonnement = de 7 configurator-opties met vt-/ext-toggles, of los product uit dezelfde vijf slugs als WS-6, geen drop-in; custom bedrag zichtbaar-uitgeschakeld), Voorwaarden (waiver + 12/24m bij abonnement, niets bij een product), Versturen (TTL 1/3/7/14 dagen, createPaymentRequest, kopieerbare link). Prijslogica uitsluitend hergebruikt uit abonnement/lib.ts (computeBreakdown, planSlug); geen nieuwe prijsformules, geen wijziging aan createPaymentRequest/findOrCreateCustomer/admin_create_order/_compute_order_price. Chrome-fix hoort bij deze PR: /betaal/[token] kreeg via SiteShell.tsx de volle marketing-navbar/footer/EM-banner mee, nu uitgesloten (zelfde patroon als /app, /login, /checkin), betaal-logica zelf ongewijzigd. Tijdens de preview-verificatie leek de betaallink-redirect na de productbetaling in eerste instantie fout omdat hij terugkeert naar /betaal/[token] in plaats van naar /app/producten?tab=tegoed; dat is GEEN bug, maar correct gedrag: de betaallink-flow (PR A) keert bewust terug naar de betaalpagina zelf omdat de klant mogelijk niet ingelogd is, terwijl de ?tab=tegoed-redirect (WS-6, create-order.ts) uitsluitend voor de ingelogde self-service-flow is. Geen fix nodig. Testdata van de round-trip (2 orders, 2 payments, 2 memberships, op de bestaande testprofielen ilja.goossens@gmail.com en marlon@ptloosdrecht.nl) is uit productie verwijderd; de Mollie-test-subscription (sub_9uuLURTSbC, mode test) is eerst bij Mollie geannuleerd en op canceled geverifieerd, pas daarna is de membership-rij opgeruimd. tmc.events ongemoeid. Vercel's build op de merge-commit is groen.
  - OPENSTAAND: betaalverzoek-overzicht als aparte vervolg-PR (lezen + statusfilter + opnieuw versturen + annuleren-van-openstaande met betaling-wint, geen verwijderen). De wizard zelf heeft geen lijstweergave van eerder verstuurde betaalverzoeken.
