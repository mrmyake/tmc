...
**Waar de telling werkelijk zit** (gecorrigeerd bij PR 5, #151, tegen de live definities):
er zijn **twee** telpaden, niet drie. `tmc.session_occupancy(p_session_id)` is de enige
telfunctie: `bookings` met status `booked`, plus `trial_bookings` met `pending`, `paid` of
`attended`, plus `guest_bookings` met `booked` of `attended`. Sinds de
guest-capaciteitsfix (`20260810`) delegeren `redeem_trial_code` én `book_class_session`
daarheen; de eerdere tekst hier dacht dat `redeem_trial_code` een eigen derde telling had.
Alleen `tmc.v_session_availability` telt nog zelf, in eigen `LATERAL`-subqueries.
...
`tmc.session_occupancy(new.session_id) >= v_capacity`.

`and not is_test` op de `trial_bookings`-subquery gaat daarom naar precies twee plekken:
`session_occupancy` (dekt de drie triggers, `redeem_trial_code` en `book_class_session` in
één keer) en `v_session_availability`.
...
**Oorspronkelijk besluit (PR #172, 2026-09-08), sinds PR #193 herzien, zie hieronder.** De
handler mocht bij een mislukte `payments.get` niet stil doorlopen. De buitenste `try/catch`
ving dat af en retourneerde `{ ok: true }`; dat bleef zo ("Mollie mag niet gaan retryen"),
met een `console.error` met het id en de gekozen modus erbij, zodat een modus-mismatch in de
logs zichtbaar was in plaats van als stilte. De uitzondering was ontbrekende configuratie:
geeft `getMollieClient(mode)` `null` terug, dan antwoordden beide webhook-routes al met 500
(sinds de live-vangrails, zie 6.6), want een 200 op een route die niets kon ophalen zou een
betalingsbevestiging definitief laten verdwijnen zodra een key wegvalt. Dat gold toen
uitsluitend voor de configuratiecheck; een mislukte `payments.get` bleef een 200.

**Herziening (PR #193, 2026-09-19, PR 2 van de webhook-betrouwbaarheid).** De discovery op
`fix/mollie-webhook-reliability` liet drie 2xx-paden zien waarop een geslaagde betaling
definitief verloren ging: een mislukte `payments.get`, een fout uit `activate_order` nadat de
payments-rij al op `paid` stond, en de buitenste catch. De regel "Mollie mag niet gaan
retryen" is daarmee achterhaald; hij beschermde tegen ruis maar kostte betalingen. Sinds deze
PR classificeert `route.ts` elke fout:

- **Transiënt wordt 500**, zodat Mollie herhaalt: netwerkfouten, Mollie 5xx en timeouts,
  Supabase- en PostgREST-fouten (lege code uit een fetch-fout, `PGRST0xx/2xx/3xx/X00`,
  SQLSTATE-klassen 08, 40, 53, 55, 57, 58 en XX). Mollie 401 en 403 tellen ook als transiënt,
  met het bestaande precedent van de 500 op ontbrekende configuratie: herhalen tot de sleutel
  klopt.
- **Permanent blijft 200**, met een luide log en een `webhook.failed`-event: Mollie 404 door
  modus-mismatch of onbekende payment, overige 4xx (400, 422), een onparseerbare body,
  `PGRST1xx` en SQLSTATE 22, 23, 42 en P0. Herhalen geeft daar per definitie dezelfde
  uitkomst.
- **Onbekend gaat naar 500.** De kosten zijn asymmetrisch: 26 uur herhalen met wat ruis is
  goedkoper dan een betaling definitief kwijtraken, en de reconciliatie van orders (PR 3)
  vangt op wat na 26 uur nog openstaat.

Toegepast op `payments.get`, `activate_order`, de buitenste catch en het subscription-pad
(aanmaken en de write van `mollie_subscription_id`, beide uit #192). Sinds PR #194 (PR 3a) staan de
classificatie en de registratie in `src/lib/orders/activation-failure.ts` en de activatieketen
zelf in `src/lib/orders/activation-chain.ts`, gedeeld met de reconciliatie van PR 3b; `route.ts`
levert alleen nog de webhook-context (bron `mollie_webhook`, 500 op `retry`).

**Kritieke pad (PR #195, 3a-bis).** De webhook antwoordt zodra `activate_order` gecommit is en het
subscription-pad is afgerond; de vijf zijeffecten (ntfy, `order.activated`, bevestigingsmail,
GA4, Akiles-sync) en de mail-retry draaien via `after()` uit `next/server` na de respons, met
`maxDuration = 60` op de route. Het subscription-pad blijft vóór de respons omdat het leunt op de
500 die Mollie laat herhalen. Alle uitgaande clients hebben sinds dezelfde PR een timeout
(`src/lib/outbound-timeouts.ts`: Supabase 8 s, Mollie 8 s, MailerSend 10 s, Akiles 4 s per call,
naast ntfy 5 s en GA4 5 s), en de functies draaien in `fra1` in plaats van `iad1`. Gevolg voor de
bevestigingsmail: mislukt hij in `after()`, dan is de 200 al weg en komt er geen webhook-retry die
de retry-tak bereikt; besluit 36 en PR 3b. Het pad "Mollie niet
geconfigureerd" was al 500 en blijft dat. De payments-upsert blijft een 200 met log: die
raakt de activatie niet.

**Mollie's retry-gedrag**, geverifieerd op https://docs.mollie.com/reference/webhooks: alleen
een `200 OK` telt als succes; een antwoord dat langer dan 15 seconden duurt telt als mislukt,
ook als er daarna alsnog een 200 komt; na een niet-200 herhaalt Mollie op 1, 2, 4, 8, 16 en
29 minuten en daarna op 1, 2 en 22 uur, tien pogingen in totaal, cumulatief 26 uur, daarna
stopt Mollie definitief. Omdat elke poging een `webhook.failed`-rij oplevert maar niet elke
poging een melding verdient, gaat er per pad en `mollie_payment_id` hooguit een ntfy uit
binnen dat venster van 26 uur; de al geschreven `webhook.failed`-rijen zijn daarvoor de bron
van waarheid, er is geen nieuwe tabel.
...
| 35 | **Herziening van 6.5 (PR #193, 2026-09-19):** transiënte fouten in de Mollie-webhook (netwerk, Mollie 5xx en timeouts, Supabase en PostgREST, en alles wat onbekend is) antwoorden 500 zodat Mollie tot 26 uur lang herhaalt; alleen fouten waarvan herhaling dezelfde uitkomst geeft (Mollie 404 en overige 4xx, onparseerbare body, SQLSTATE 22/23/42/P0) blijven 200 met een `webhook.failed`-event. Per pad en `mollie_payment_id` hooguit een ntfy per 26 uur, met de `webhook.failed`-rijen als bron. | De 200 op elke fout uit PR #172 ("Mollie mag niet gaan retryen"). Verworpen: de discovery op `fix/mollie-webhook-reliability` toonde drie 2xx-paden waarop een geslaagde betaling definitief verloren ging; 26 uur herhalen met ruis is goedkoper dan een betaling kwijtraken, en PR 3 (reconciliatie) vangt op wat na 26 uur openstaat. Ook verworpen: onbekende fouten op 200 houden, want dat is precies de klasse waarin een nieuw stil pad ontstaat. |
| 36 | **De bevestigingsmail heeft sinds 3a-bis (PR #195, 2026-09-19) geen vangnet meer in de webhook.** Hij draait in `after()`, na de 200; mislukt hij daar (MailerSend down, timeout van 10 s), dan komt er geen tweede webhook die de retry-tak met `order.confirmation_sent` als poort bereikt. Vastgelegd, niet gebouwd: PR 3b (reconciliatie in `expire-orders`) controleert bij elke gereconcilieerde of al geactiveerde order ook `order.confirmation_sent` en verstuurt de mail alsnog via dezelfde `sendOrderConfirmation`, met dezelfde poort. | De mail synchroon vóór de respons houden. Verworpen: de mail is 1 tot 1,6 s op het happy path en tot 10 s bij een dode MailerSend, binnen Mollie's 15 s de grootste post na Akiles, en een mislukte mail mag de betaalbevestiging richting Mollie niet ophouden. |
...
- **PR 5, #151, 2026-08-07** (migratie `20260820000000_trial_booking_mode.sql` plus vier
  TypeScript-bestanden).
  De proefles krijgt een eigen testmodus en wordt zichtbaar in de omzet:
  `trial_bookings.is_test` (gezet door de aanmaakroute uit `trialBookingMode()`),
  `and not is_test` in `session_occupancy` en `v_session_availability`, de trial-webhook
  schrijft elke statusovergang naar `tmc.payments` met `kind = 'trial_booking'`,
  `trial_booking_id`, `is_test` uit de trial-rij en het BTW-snapshot tegen het
  `drop_in`-tarief, backfill van de twee bestaande betaalregels met de echte
  Mollie-statussen als constante (`paid`/iDEAL en `expired`, read-only opgehaald), en de
  `expire-orders`-TODO uit PR 4 ingelost (modus per rij). Ook
  `admin/rooster/page.tsx` filtert testrijen, zodat Marlon hetzelfde getal ziet als de
  handhaving telt.
  **Spec-correctie in 2.9 en 14:** `redeem_trial_code` en `book_class_session` delegeren
  sinds `20260810` al naar `session_occupancy`; het zijn twee telpaden, niet drie.
  **Bewust niet gebouwd:** een tweede bovengrens voor testrijen. De asymmetrie uit
  besluitenlog 25 is in de migratie-zelfcontrole expliciet nagespeeld op een synthetische
  sessie met capacity 2 (binnen de transactie aangemaakt en verwijderd): testrijen tellen
  niet en begrenzen elkaar niet, echte rijen tellen wel, de derde echte weigert op
...
- **PR #192, 2026-09-19** (branch `fix/mollie-webhook-guards`, geen migratie; `src/app/api/mollie/webhook/route.ts`, `src/lib/events/emit.ts`). PR 1 van de webhook-betrouwbaarheid (discovery op `fix/mollie-webhook-reliability`): idempotentie-guards zonder wijziging van de statuscodes. Een bestaande Mollie-subscription voor het membership wordt hergebruikt in plaats van een tweede aan te maken (de idempotencyKey dekt een uur, Mollie's retries lopen 26 uur), de write van `mollie_subscription_id` wordt gecontroleerd en een mislukking daarvan wordt persistent vastgelegd met ntfy, de PT-tak vuurt ntfy en `pt_booking.confirmed` alleen op de echte status-flip, een retry op een eerder geblokkeerde order geeft geen ntfy "Order-activatie geweigerd" meer, en vier faalpaden (`payments.get`, `activate_order`, de subscription-write en de buitenste catch) schrijven een `webhook.failed`-event naar `tmc.events`. Het type `webhook.failed` is live geverifieerd: `tmc.events.type` is kale `text` zonder check, domein of enum (alleen `events_actor_type_check` en de append-only trigger bestaan), en een insert in een teruggedraaide transactie werd geaccepteerd. De dedupe van `payment.received` is bewust niet gebouwd; optie b is gekozen (duplicaten accepteren, lezers dedupliceren; vandaag leest geen enkele lezer `payment.*`, ook `vw_admin_kpis` niet). Het migratiebestand voor optie a is van de branch verwijderd omdat een `db push` het anders zou meenemen en daarbij een rij uit de append-only log zou wissen; komt optie a ooit aan de orde, dan wordt hij opnieuw geschreven. Bekend restrisico, alleen vastgelegd: een membership waarvan `mollie_subscription_id` nooit is weggeschreven en waarvan de subscription daarna handmatig bij Mollie is geannuleerd, krijgt bij een retry een nieuwe subscription; smal pad, want `needs_subscription` vereist een null id. De ntfy op het `subscription_link_write`-pad moet onder PR 2 herzien worden: retries maken er tien meldingen voor één probleem van, het event mag herhalen, de melding niet. **Bewust niet aangeraakt:** de 2xx-afhandeling en de passage in 6.5 over de 200 op `payments.get` (PR 2), `activate_order` en alle andere RPC's, de reconciliatie in `expire-orders` (PR 3), de factuurketen.

- **PR #193, 2026-09-19** (branch `fix/mollie-webhook-non-2xx`, geen migratie; `src/app/api/mollie/webhook/route.ts`, dit document 6.5, 12 en 15). PR 2 van de webhook-betrouwbaarheid: foutclassificatie in de webhook (transiënt wordt 500 zodat Mollie herhaalt, permanent blijft 200 met `webhook.failed`, onbekend gaat naar 500), toegepast op `payments.get`, `activate_order`, de buitenste catch en het subscription-pad uit #192 (aanmaken en de write van `mollie_subscription_id`); een onparseerbare body krijgt een eigen `malformed_body`-registratie; de ntfy op alle faalpaden wordt onderdrukt tot hooguit een per pad en `mollie_payment_id` binnen het retry-venster van 26 uur, met de `webhook.failed`-rijen als bron van waarheid; 6.5 herzien met Mollie's retry-schema, besluit 35 toegevoegd. Stap 0 vooraf: de zijeffecten achter `!already_activated` zijn allemaal throw-vrij, de bevestigingsmail heeft een eigen poort, GA4 is bewust at-most-once, en de nachtelijke Akiles-sync (02:15 UTC, `needsAkilesUpdate` bij ontbrekende credentials) repareert ontbrekende toegang binnen 24 uur; daarom geen extra poorten. **Bewust niet aangeraakt:** `activate_order` en alle andere RPC's, de payments-upsert (blijft 200 met log), de reconciliatie in `expire-orders` (PR 3), de factuurketen.

- **PR #194, 2026-09-19** (branch `refactor/activation-chain-extract`, geen migratie; `src/lib/orders/activation-chain.ts` en `src/lib/orders/activation-failure.ts` nieuw, `src/app/api/mollie/webhook/route.ts`, `src/lib/ntfy.ts`, `scripts/ntfy/ntfy.test.mts`, dit document 6.5 en 15). PR 3a van de webhook-betrouwbaarheid: de activatieketen (aanroep van `activate_order`, afhandeling van `already_activated` en `invalid_status`, de vijf zijeffecten achter `!already_activated`, de retry-tak van de bevestigingsmail en het subscription-pad) is zonder gedragswijziging uit de webhook getild naar `runActivationChain(caller, input)`, met een expliciete aanroeper-context (`source`: `mollie_webhook` of `expire_orders_cron`, landt op `webhook.failed`; `actor_type` altijd `system`) en zonder afhankelijkheid van een Request, zodat PR 3b hem vanuit `expire-orders` kan aanroepen. `registerFailure` en `classifyFailure` zijn mee verhuisd en worden door beide aanroepers gedeeld. De keten logt per stap de duur (`[activation-chain] timing`) als meting tegenover Mollie's 15 seconden. Los daarvan, eigen commit: `NTFY_URL` wordt op Vercel-productie genegeerd. **Bewust niet aangeraakt:** de responscodes en de classificatie uit #193, `activate_order` en alle andere RPC's, de reconciliatie-cron zelf (PR 3b), en het verplaatsen van zijeffecten naar `after()` (aanbeveling in de PR-body, eigen PR).

- **PR #195, 2026-09-19** (branch `fix/webhook-critical-path`, geen migratie; `vercel.json`, `src/lib/outbound-timeouts.ts` nieuw, `src/lib/supabase/admin.ts` en `server.ts`, `src/lib/mollie.ts`, `src/lib/email.ts`, `src/lib/akiles.ts`, `src/lib/orders/activation-chain.ts`, `src/app/api/mollie/webhook/route.ts`, `scripts/outbound/timeouts.test.mts`, dit document 6.5, 12 en 15; cross-cutting regels in `spec-membership-flow.md`). 3a-bis van de webhook-betrouwbaarheid, drie commits: (1) functies vastgezet op `fra1` na vaststelling met `vercel inspect` dat ze in `iad1` draaiden, gemeten hop 0,2 tot 0,3 s per databasecall; (2) expliciete timeouts op Supabase, Mollie, MailerSend en Akiles, elk als gevangen fout van die dienst; (3) de vijf zijeffecten en de mail-retry achter de respons met `after()`, `maxDuration = 60`, subscription-pad synchroon, `runActivationChain` met `options.defer` zodat een cron hem inline blijft draaien. Het kritieke pad na (3) is **samengesteld uit losse latentiemetingen, niet end-to-end gemeten**: 0,4 tot 0,9 s voor een abonnement en 0,05 tot 0,3 s voor een product; de eerste test-mode-betaling op een preview moet die cijfers via de `phase: "critical"`-timing-log bevestigen voordat ze als gemeten gelden. Over de Supabase-timeout van 8 s: die valt samen met de server-side `statement_timeout` van 8 s voor al het PostgREST-verkeer (rol `authenticator`, door `service_role` geërfd; live `pg_roles.rolconfig` en Supabase-docs), en `pg_stat_statements` sinds 2026-04-06 toont geen enkele PostgREST-call boven 34 ms (zwaarste: `refresh_admin_kpis`), dus er is geen per-call override aangebracht; een client-side abort annuleert de query niet aan de databasekant, wat voor `activate_order` door de idempotentie onschadelijk is en voor `refresh_admin_kpis` hooguit een 500 in de cron-log oplevert terwijl de refresh geslaagd is (`outbound-timeouts.ts`). Mollie-schrijfacties die bij een timeout een resource achterlaten waar wij geen weet van hebben (`payments.create` in `create-order.ts`, `payment-link-core.ts` en `trial-booking.ts`; `customers.create` in `create-order.ts` en `payment-link.ts`) neemt PR 3b mee in de reconciliatie; subscriptions zijn al gedekt door de guard uit #192. Besluit 36: de bevestigingsmail heeft daarmee geen webhook-vangnet meer, PR 3b pakt dat op. **Bewust niet aangeraakt:** de classificatie en responscodes uit #193, `activate_order` en alle andere RPC's, de reconciliatie-cron (PR 3b).
