...
export type ProductenView = "kopen" | "tegoed";
...
export function ProductenTabs({ active }: { active: ProductenView }) {
  return (
    <nav
      role="tablist"
      aria-label="Producten-weergave"
      className="flex items-center gap-10 border-b border-[color:var(--ink-500)]/50"
    >
      {/* COPY: confirm met Marlon */}
      <Tab label="Kopen" active={active === "kopen"} href="/app/producten" />
      {/* COPY: confirm met Marlon */}
      <Tab
        label="Mijn tegoed"
        active={active === "tegoed"}
        href="/app/producten?tab=tegoed"
      />
    </nav>
  );
}
