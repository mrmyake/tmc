...
- **Klantbeheer-workstream**: adres bijwerken, abonnement pauzeren/stopzetten, EN de geparkeerde directe e-mailcorrectie op een bestaand account (uit WS-5 betaalverzoek-overzicht PR 2: auth.users.email + identities + profiles.email synchroon, zoek-eerst, weigeren bij bestaand ander account, volledige PR B-strengheid; wijzigt een login-adres, dus Fable). Eigen discovery; pauzeren/stoppen is Fable (mandaat-wijziging).
