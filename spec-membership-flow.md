...
- **Klantbeheer-workstream**: fase 1 (pauzeren/hervatten als gedeelde RPC-laag) is GEMERGED (PR #88, `688b5ba`, zie de ledger-regel hierboven); discovery in `discovery-klantbeheer-lifecycle.md` (PR #87). Resteert fase 2 (na expliciete go): admin-stop (`admin_cancel_membership`, terminaal, met gemarkeerde hard-stop-parameter), `membership_change_requests` (alleen-upgrade op de volgende factuurdatum, geen proratie), en de geparkeerde directe e-mailcorrectie op een bestaand account (auth.users.email + identities + profiles.email synchroon, zoek-eerst, weigeren bij bestaand ander account, volledige PR B-strengheid; wijzigt een login-adres, dus Fable). Adres bijwerken bestaat al (`updateProfile`, puur profielveld, geen Mollie-operatie).
