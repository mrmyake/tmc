...
- **Profiel** (`/app/profiel`) — BESTAAT. Gegevens, health intake, marketing opt-in, account
  verwijderen. Sinds workstream D.2 in `spec-ios-app.md` (PR #206-#208) is dit een echte flow op
  `/app/profiel/verwijderen`: preflight, OTP-bevestiging, en daadwerkelijke sluiting (toegang,
  boekingen, tegoed) op de dag na de laatste betaalde dag van een opgezegd abonnement, of direct
  zonder lopend abonnement. Geen "soft delete + grace" in de zin van een losstaande vlag: de
  status leeft in `tmc.account_deletions`, met purge door de nachtelijke cron.
