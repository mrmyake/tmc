...
- **PR #186** (2026-09-10): PIN-invoer op `/checkin` verstuurt automatisch zodra het volledige aantal cijfers is ingevoerd (`AdminLockScreen`), geen bevestigknop meer; PIN-lengte vast op 6 cijfers (was vrij 4-6, nergens apart vastgelegd) via de nieuwe `CHECKIN_PIN_LENGTH`-constante, ook afgedwongen bij het instellen (`CheckinPinForm`, de `set_admin_checkin_pin`-RPC via migratie `20260911120000`, inmiddels gepusht). Tijdens verificatie geblokkeerde invoer + zichtbare wachtstatus; bij een foute PIN auto-clear + foutmelding + focus terug; nieuwe "Wissen"-knop voor een volledige reset halverwege. Bewust niet aangeraakt: de brute-force-rem zelf (`checkin_pin_attempts`, 10 pogingen/15 min lockout, ongewijzigd) — wel gerapporteerd dat auto-submit de foutmarge verkleint omdat een tikfout niet meer voor te bekijken/corrigeren is vóór verzending.
- **PR #187** (2026-09-10): nieuwe route `/kiosk`, een vandaag-weergave voor de muurtablet, tweede ingang naar de bestaande deelnemerslijst (`/app/trainer/sessies/[id]`, `MobileAttendanceList`, `markAttendance()`) — geen nieuwe incheck-logica. Zelfde auth-/rolcheck als `/app/trainer/**`; admin ziet alle sessies van vandaag, een trainer uitsluitend de eigen via `resolveTrainerScope()`. Toont de actieve les ("Nu bezig") of de eerstvolgende ("Straks"), een rustige lege staat als er niks te tonen is, en de volledige dagkolom met per sessie `{ingecheckt}/{geboekt}` — ingecheckt-aantal uit `tmc.check_ins.session_id`, bevestigd correct omdat sessie-gebonden check-ins na PR #185 uitsluitend via `markAttendance()` ontstaan. Layout volgt `stitch/kiosk-vandaag.html` (goedgekeurde mockup) exact, vertaald naar een scoped CSS module; uitgesloten van robots.txt en marketing-chrome zoals `/checkin`. Bewust niet aangeraakt: `MobileAttendanceList`/`markAttendance()`/`loadParticipants()`, `resolveTrainerScope()` zelf, de site-brede font-load. Gerapporteerd, niet zelf gewijzigd: Fraunces-gewicht 300 in de mockup staat haaks op de bestaande leesbaarheidsafweging in `tokens.css` ("Fraunces needs weight 400+ to stay sharp"); twee mockup-kleuren (`--surface`, `--surface-lift`) wijken een paar hex-cijfers af van de dichtstbijzijnde bestaande tokens. Fysieke-tablet-test (Fully Kiosk Browser, 1340x800) nog niet gedaan, geen toegang tot het device.
